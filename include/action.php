...
if($id =="login")
{
	      ...
	      $password =  base64_encode($_POST['password']);
			$select=mysqli_query($con,"SELECT * FROM admin_user WHERE emailid='$email' AND password='$password' and status=1");
			$num_rows= mysqli_num_rows($select);
			$fetchquery = mysqli_fetch_array($select);
			if($num_rows == 1) {
				$_SESSION['login'] = true;
				$_SESSION['username'] = $fetchquery['firstname'];	
				$_SESSION['email'] = $fetchquery['emailid'];	
				$_SESSION['id'] = $fetchquery['id'];
				if($fetchquery['roletype'] ==2)
				{
				echo "<script>window.location.href = '../upload.php';</script>";
				}
		else{
			echo "<script>window.location.href = '../procurement.php?file=general&st=procurement';</script>";
		}	
			} 
...
if($id =="logout")
{
unset($_SESSION['login']); unset($_SESSION['username']); unset($_SESSION['id']);
echo "<script>window.location.href = '../login_bescom.php';</script>";
}	
